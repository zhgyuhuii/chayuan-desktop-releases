...
class PlatformToolsMessageChunk(AIMessage, BaseMessageChunk):
    ...
    def __add__(self, other: Any) -> BaseMessageChunk:  # type: ignore
        if isinstance(other, PlatformToolsMessageChunk):
            self_example = getattr(self, "example", None)
            other_example = getattr(other, "example", None)
            if self_example != other_example:
                raise ValueError(
            ...
            if self_example is not None:
                init_kwargs["example"] = self_example
