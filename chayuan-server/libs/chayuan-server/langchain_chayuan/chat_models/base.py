...
from chayuan.server.pydantic_v1 import Field, SecretStr, root_validator
...
from langchain_chayuan.chat_models.platform_tools_message import (
...
class ChatPlatformAI(BaseChatModel):
    ...
    def lc_secrets(self) -> Dict[str, str]:
        return {"chayuan_api_key": "CHAYUAN_API_KEY"}
    ...
    def lc_attributes(self) -> Dict[str, Any]:
        ...
        if self.chayuan_api_base:
            attributes["chayuan_api_base"] = self.chayuan_api_base

        if self.chayuan_proxy:
            attributes["chayuan_proxy"] = self.chayuan_proxy
    ...
    chayuan_api_key: Optional[SecretStr] = Field(default=None, alias="api_key")
    """Automatically inferred from env var `CHAYUAN_API_KEY` if not provided."""
    chayuan_api_base: Optional[str] = Field(default=None, alias="api_base")
    """Base URL path for API requests, leave blank if not using a proxy or service 
        emulator."""
    # to support explicit proxy for OpenAI
    chayuan_proxy: Optional[str] = Field(default=None, alias="proxy")
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        # get_from_dict_or_env uses str(value); SecretStr.__str__ is masked ("**********"),
        # which would be sent to the API and cause bogus 401s.
        raw_api_key = values.get("chayuan_api_key") or values.get("api_key")
        if raw_api_key is not None and hasattr(raw_api_key, "get_secret_value"):
            api_key_plain = raw_api_key.get_secret_value()
        elif raw_api_key:
            api_key_plain = str(raw_api_key)
        else:
            api_key_plain = get_from_dict_or_env(
                values, "chayuan_api_key", "CHAYUAN_API_KEY"
            )

        values["chayuan_api_key"] = convert_to_secret_str(api_key_plain)

        values["chayuan_api_base"] = values["chayuan_api_base"] or os.getenv(
            "CHAYUAN_API_BASE"
        )
        values["chayuan_proxy"] = get_from_dict_or_env(
            values,
            "chayuan_proxy",
            "CHAYUAN_PROXY",
            default="",
        ...
        client_params = {
            "api_key": (
                values["chayuan_api_key"].get_secret_value()
                if values["chayuan_api_key"]
                else None
            ),
            "base_url": values["chayuan_api_base"],
