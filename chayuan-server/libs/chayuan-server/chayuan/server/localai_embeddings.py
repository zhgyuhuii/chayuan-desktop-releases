...
from chayuan.server.pydantic_v1 import BaseModel, Field, root_validator
...
from chayuan.server.utils import run_in_thread_pool
...
def _breaker():
    """P2-7：LocalAI embedding 专用 breaker；缺 resilience 时退化 None（no-op）。"""
    try:
        from chayuan.server.resilience import get_breaker
        return get_breaker("localai-embeddings")
    except Exception:  # noqa: BLE001
        return None


def embed_with_retry(embeddings: LocalAIEmbeddings, **kwargs: Any) -> Any:
    """Use tenacity to retry the embedding call; P2-7 再叠加 circuit breaker。"""
...
async def async_embed_with_retry(embeddings: LocalAIEmbeddings, **kwargs: Any) -> Any:
    ...
    if b is not None:
        b.before_call()
    try:
        res = await _async_embed_with_retry(**kwargs)
    except BaseException:
        if b is not None:
            b.on_failure()
        raise
...
class LocalAIEmbeddings(BaseModel, Embeddings):
    ...
    class Config:
        ...
        # Pydantic v2 用 validate_by_name 替代 v1 的 allow_population_by_field_name;
        # v2 仍读旧名但每次启动会 UserWarning,显式写新名消掉。
