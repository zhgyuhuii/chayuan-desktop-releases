...
from chayuan.server.file_rag.text_splitter import (
    zh_title_enhance as func_zh_title_enhance,
)
from chayuan.server.utils import run_in_process_pool, run_in_thread_pool
...
def validate_kb_name(knowledge_base_id: str) -> bool:
    # KB 名称是用户可见名称,允许中文和常见符号；这里只拦截路径穿越/控制字符。
    if not isinstance(knowledge_base_id, str):
        return False
    if not knowledge_base_id.strip():
        return False
    if len(knowledge_base_id) > 200:
        return False
    if any(ch in knowledge_base_id for ch in ("\x00", "/", "\\")):
        return False
    if any(ord(ch) < 32 for ch in knowledge_base_id):
        return False
    return True


def new_vector_namespace(length: int = 20) -> str:
    """生成向量库内部 collection/index 名。

    不从用户名称翻译/转写,避免重名、模型依赖和改名影响；只使用 ASCII 小写字母,
    兼容 Milvus / Chroma / ES / PGVector 等后端的保守命名约束。
    """
    alphabet = string.ascii_lowercase
    return "".join(secrets.choice(alphabet) for _ in range(max(1, length)))


def safe_kb_fs_name(kb_name: str, *, prefix: str = "kbfs_") -> str:
    """把用户可见 KB 名映射为跨平台安全的本地目录名。

    Windows 禁用 ``<>:"/\\|?*`` 和尾随空格/点；Linux 虽宽松,但服务可能在
    Windows 部署。合法 ASCII 旧库保持原路径,其它名称用 sha1 稳定映射。
    """
    import hashlib
    import re as _re

    if _re.match(r"^[A-Za-z0-9_. -]{1,120}$", kb_name) and not kb_name.endswith((" ", ".")):
        return kb_name
    return f"{prefix}{hashlib.sha1(kb_name.encode('utf-8')).hexdigest()[:24]}"


def safe_vs_collection_name(kb_name: str, *, prefix: str = "kb_", max_len: int = 255) -> str:
    """把任意 kb_name 映射成向量库 (Milvus / Zilliz / Milvus Lite) 合法的 collection 名。

    Milvus 的硬约束：``^[A-Za-z_][A-Za-z0-9_]{0,254}$``——不允许中文、连字符、空格等。
    我们 UI 上允许用户用中文（"成语大全"）给知识库起名，所以到了向量库这一层必须做
    一次确定性映射：

    - 纯 ASCII ``[A-Za-z_][A-Za-z0-9_]*`` 且长度达标 → 原样透传（最佳可读性，兼容老库）；
    - 否则 → ``<prefix><sha1(kb_name)[:24]>``，稳定且唯一。

    这是纯函数：同一个 kb_name 永远产出同一个 collection 名，查询/删除都能对回来。
    """
    import hashlib
    import re as _re
    ...
    # 合法则原样透传，保留与存量向量库的兼容性。
    if _re.match(rf"^[A-Za-z_][A-Za-z0-9_]{{0,{max_len - 1}}}$", kb_name):
        return kb_name

    # 否则用 sha1 前缀做确定性映射；24 个 hex 字符 → 96 bit，冲突概率对业务量级可忽略。
    digest = hashlib.sha1(kb_name.encode("utf-8")).hexdigest()[:24]
    name = f"{prefix}{digest}"
    # 保险起见再 clamp 一下长度（prefix 太长时）。
    return name[:max_len]


def get_kb_path(knowledge_base_name: str):
    return os.path.join(Settings.basic_settings.KB_ROOT_PATH, safe_kb_fs_name(knowledge_base_name))
...
LOADER_DICT = {
    ...
    "MHTMLLoader": [".mhtml"],
    # 纯文本/markdown/常见配置 / 代码文件 全部走 TextLoader:不走 Unstructured/NLTK,
    # 无 punkt 依赖,解析 1MB txt 比 Unstructured 快 ~50 倍且零外部下载需求。
    # 配置文件(.yml/.yaml/.toml/.ini/.cfg/.env/.properties)和代码文件(.py/.js/...)
    # 都是文本读、按行/段切分,没必要走 Unstructured 重路径。
    "TextLoader": [
        ".md", ".markdown", ".mdx",
        ".txt", ".log",
        # 配置 / 元数据
        ".yaml", ".yml", ".toml", ".ini", ".cfg", ".conf", ".env", ".properties",
        # 代码 — 上传到 KB 是为了 RAG/搜索,纯文本切块就够
        ".py", ".js", ".ts", ".tsx", ".jsx",
        ".go", ".rs", ".java", ".kt", ".scala", ".swift",
        ".c", ".h", ".cpp", ".hpp", ".cc", ".cxx", ".cs",
        ".php", ".rb", ".pl", ".lua", ".sh", ".bash", ".zsh", ".fish",
        ".sql", ".graphql", ".proto",
        ".html", ".htm", ".css", ".scss", ".less", ".vue", ".svelte",
        ".dockerfile",  # 注意:无扩展名的 "Dockerfile" 走不到这里,前端按需 rename
    ],
...
def get_loader(loader_name: str, file_path: str, loader_kwargs: Dict = None):
    ...
    try:
        if loader_name in [
            ...
            document_loaders_module = importlib.import_module(
                "chayuan.server.file_rag.document_loaders"
            )
        else:
            document_loaders_module = importlib.import_module(
    ...
    if loader_name == "UnstructuredFileLoader":
        ...
        # 中文 .txt / .md 常见 GBK / GB18030 编码;不打开 autodetect_encoding 会
        # UnicodeDecodeError 直接挂掉整个 KB 入库链。这里强制 autodetect。
        loader_kwargs.setdefault("autodetect_encoding", True)
    ...
    try:
        if (
            ...
            try:  # 优先使用用户自定义的text_splitter
                text_splitter_module = importlib.import_module("chayuan.server.file_rag.text_splitter")
...
class KnowledgeFile:
    ...
    def file2docs(self, refresh: bool = False):
        if self.docs is None or refresh:
            ...
            # TextLoader 在 get_loader 里已经 autodetect_encoding=True;
            # 之前硬覆盖 loader.encoding="utf8" 会让 GBK / GB18030 的中文 .txt 直接
            # UnicodeDecodeError,反而不如 autodetect 稳。这里统一一条 load()。
...
def format_reference(
    ...
    - `access_token`：可选的 JWT，会作为 ``?token=<jwt>`` 拼进下载链接。
      这是给「浏览器点击 a 标签下载」的场景用的：浏览器发 GET 没法带
      Authorization header，后端 AuthMiddleware 对 download_doc 这个白名单路径
      会回退读 query 里的 token，保证鉴权不被绕过。
    '''
