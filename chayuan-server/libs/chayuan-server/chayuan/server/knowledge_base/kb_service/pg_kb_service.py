...
class PGKBService(KBService):
            ...
            session.execute(
                text(
                    ...
                    WHERE collection_id IN (
                      SELECT uuid FROM langchain_pg_collection WHERE name = '{self.vector_namespace}'
                    );
                    -- 删除 langchain_pg_collection 表中 记录
                    DELETE FROM langchain_pg_collection WHERE name = '{self.vector_namespace}';
            """
                )
            )
            session.commit()
    ...
    def do_delete_doc(self, kb_file: KnowledgeFile, **kwargs):
        ...
        """)
        with Session(PGKBService.engine) as session:
            collection_id = session.execute(select_query, {"name": self.vector_namespace}).fetchone()[0]
            session.execute(
                delete_query, 
                {
