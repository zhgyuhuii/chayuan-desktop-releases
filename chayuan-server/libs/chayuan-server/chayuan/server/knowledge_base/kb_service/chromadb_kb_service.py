...
# chromadb 为可选重依赖：只有用户选择 chromadb 作为向量库时才需要。
# 这里做一次按需自动安装，避免第一次使用时直接报 ImportError。
try:
    from chayuan.server.shared.deps import ensure_pkg
    ensure_pkg("chromadb", "chromadb>=0.4,<0.6")
    ensure_pkg("langchain_chroma", "langchain-chroma>=0.1,<1.0")
...
class ChromaKBService(KBService):
    ...
    def do_init(self) -> None:
        ...
        collection = self.client.get_or_create_collection(self.vector_namespace)
        self._load_chroma()
    ...
    def do_drop_kb(self):
        ...
        try:
            self.client.delete_collection(self.vector_namespace)
        except ValueError as e:
            if not str(e) == f"Collection {self.vector_namespace} does not exist.":
                raise e
