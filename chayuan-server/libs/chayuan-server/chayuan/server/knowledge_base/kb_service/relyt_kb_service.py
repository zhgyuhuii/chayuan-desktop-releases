...
class RelytKBService(KBService):
    def _load_relyt_vector(self):
        ...
        self.relyt = PGVecto_rs(
            ...
            collection_name=self.vector_namespace,
        )
    ...
    def get_doc_by_ids(self, ids: List[str]) -> List[Document]:
        ...
        with Session(self.engine) as session:
            stmt = text(
                f"SELECT text, meta FROM collection_{self.vector_namespace} WHERE id in (:ids)"
            )
            results = [
    ...
    def del_doc_by_ids(self, ids: List[str]) -> bool:
        ids_str = ", ".join([f"{id}" for id in ids])
        with Session(self.engine) as session:
            stmt = text(f"DELETE FROM collection_{self.vector_namespace} WHERE id in (:ids)")
    ...
    def do_create_kb(self):
        index_name = f"idx_{self.vector_namespace}_embedding"
                ...
                if not result:
                    index_statement = text(
                        f"""
                            ...
                            ON collection_{self.vector_namespace}
    ...
    def do_drop_kb(self):
        drop_statement = text(f"DROP TABLE IF EXISTS collection_{self.vector_namespace};")
        with self.engine.connect() as conn:
            with conn.begin():
    ...
    def do_delete_doc(self, kb_file: KnowledgeFile, **kwargs):
        ...
        stmt = f"DELETE FROM collection_{self.vector_namespace} WHERE meta->>'source'='{filepath}'; "
        with Session(self.engine) as session:
