...
def strip_sql_fences_for_execute(conn, cursor, statement, parameters, context, executemany):
    from chayuan.server.knowledge_source.sql.text2sql import strip_sql_fences

    return strip_sql_fences(statement or ""), parameters


def intercept_sql(conn, cursor, statement, parameters, context, executemany):
    cleaned_statement, parameters = strip_sql_fences_for_execute(
    ...
    if any(cleaned_statement.strip().lower().startswith(op) for op in write_operations):
...
def query_database(query: str, config: dict):
    ...
    # streaming=False:LangChain SQLDatabaseChain 内部串行 3 次 LLM round-trip
    # (只读判断 / SQL 生成 / 自然语言总结),输出都是短文本,不需要流式;
    # 远端 LLM 兼容服务 (ollama / proxy) 的 SSE 转发常常中段断流,抛
    # openai.APIError("An error occurred during streaming")。改非流式可避免。
    llm = get_ChatOpenAI(
    ...
    if read_only:
        ...
        event.listen(db._engine, "before_cursor_execute", intercept_sql, retval=True)
    else:
        event.listen(db._engine, "before_cursor_execute", strip_sql_fences_for_execute, retval=True)
    ...
    if intermediate_steps:
        if len(intermediate_steps) > 2:
            ...
            sql_detail = strip_sql_fences(sql_detail)
...
def text2sql(
    ...
    """用自然语言提问数据库 — LLM 自动转 SQL 只读查询并执行。
调用时机:用户问「统计/查询数据库里的 X」「最近 N 天的 Y」「按 Z 分组求总数/均值」等结构化数据问题。
输入:中文/英文自然语言数据问题(尽量明确表名、时间范围、分组维度,LLM 能生成更稳的 SQL)。
输出:SQL 语句 + 查询结果表(行数过多会被截断)。
不要用于:文档检索(用本地知识库)、写操作 / DDL(本工具仅 SELECT)、未配置数据源时、监控指标(用 text2promql)。"""
