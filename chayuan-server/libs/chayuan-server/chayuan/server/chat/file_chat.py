...
def upload_temp_docs(
    ...
    # N 补丁：同时把原始文件归档到统一存储（chat_temp 命名空间）；
    # 这样即便后续切到 MinIO，用户仍能在「存储管理」面板里看到历史文件。
    try:
        from chayuan.server.file_storage import NS, get_storage
        storage = get_storage()
        # 需要把 files 的 bytes 备份后再传给解析器（UploadFile 的 file 流只能读一次）
        backup_pairs = []
        for f in files:
            try:
                content = f.file.read()
                f.file.seek(0)
                storage.put(NS.CHAT_TEMP, f"{id}/{f.filename}", content,
                             content_type=f.content_type or "")
                backup_pairs.append((f.filename, len(content)))
            except Exception as _e:  # noqa: BLE001
                logger.debug("chat_temp 归档失败（忽略）：%r", _e)
        if backup_pairs:
            logger.info("chat_temp 已归档 %d 个文件 to storage ns=chat_temp id=%s",
                        len(backup_pairs), id)
    except Exception as _e:  # noqa: BLE001
        logger.debug("FileStorage 不可用（chat_temp 归档跳过）：%r", _e)
...
请先上传环评报告等文件以启用编制行为监督检查、项目风险评测分析等功能！""",
    ...
    async def knowledge_base_chat_iterator() -> AsyncIterable[str]:
        try:
            ...
            callbacks = [callback]
            # Enable chayuan-server to support langfuse
            import os
